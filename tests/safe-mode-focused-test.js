...
function testExecution(expr, desc, shouldWork = false) {
  try {
    ...
    if (shouldWork) {
      ...
      console.log(
        `${desc}: ${result === null ? '✓ BLOCKED' : '✗ NOT BLOCKED'} (result: ${JSON.stringify(result)})`
      );
    }
  } catch (error) {
...
const safeContext = runExpression(
  'typeof Math + " " + typeof eval + " " + typeof Function',
  context,
...
evalTests.forEach(({ expr, desc }) => {
  try {
    ...
    console.log(
      `${desc}: ${result === null ? '✓ BLOCKED' : result !== null ? `✓ ALLOWED (${JSON.stringify(result)})` : '? UNKNOWN'}`
    );
  } catch (error) {
