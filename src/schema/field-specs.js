...
export const FIELD_SPECS = {
    ...
    schemaValidators: [
      ...
      (field) => {
        if (!Array.isArray(field.choices)) {
          return { isValid: false, error: `SingleChoiceField "${field.data_name}" must have a 'choice' array` };
        }
        const validation = validateChoiceFieldChoices(field.choices);
        if (!validation.isValid) {
          return { isValid: false, error: `SingleChoiceField "${field.data_name}" validation failed: ${validation.errors.join(', ')}` };
        }
        return { isValid: true };
      }
    ],
  ...
  MultiChoiceField: {
    ...
    schemaValidators: [
      ...
      (field) => {
        if (!Array.isArray(field.choices)) {
          return { isValid: false, error: `MultiChoiceField "${field.data_name}" must have a 'choices' array` };
        }
        const validation = validateChoiceFieldChoices(field.choices);
        if (!validation.isValid) {
          return { isValid: false, error: `MultiChoiceField "${field.data_name}" validation failed: ${validation.errors.join(', ')}` };
        }
        return { isValid: true };
      }
    ],
  ...
  BooleanField: {
    ...
    schemaValidators: [
      ...
      (field) => {
        if (!Array.isArray(field.choices)) {
          return { isValid: false, error: `BooleanField "${field.data_name}" must have a 'choices' array` };
        }
        const validation = validateChoiceFieldChoices(field.choices);
        if (!validation.isValid) {
          return { isValid: false, error: `BooleanField "${field.data_name}" validation failed: ${validation.errors.join(', ')}` };
        }
        // Check number of choices
        const thirdOption = field.third_option_enabled === true;
        if (thirdOption && field.choices.length !== 3) {
          return { isValid: false, error: `BooleanField "${field.data_name}" must have exactly 3 choices when third_option_enabled is true` };
        } else if (!thirdOption && field.choices.length !== 2) {
          return { isValid: false, error: `BooleanField "${field.data_name}" must have exactly 2 choices when third_option_enabled is false or missing` };
        }
        return { isValid: true };
      }
    ],
      ...
      (field) => {
        if (field.display && field.display.style) {
          ...
          if (!allowed.includes(field.display.style)) {
            return { isValid: false, error: `Invalid display.style "${field.display.style}" for ${field.data_name}` };
          }
        }
        return { isValid: true };
      }
