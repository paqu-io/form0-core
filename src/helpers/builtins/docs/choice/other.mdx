---
title: "OTHER Builtin"
description: "Retrieve the other option value from choice field (ChoiceField or MultiChoiceField)"
category: "choice"
...
The `OTHER` builtin retrieves the other label if the user entered an other option in a choice field, returning `null` if no other option is present. Works with both ChoiceField (single selection) and MultiChoiceField (multiple selections).
...
### Custom Input Display - ChoiceField
